...
int LED1 = 22, LED2 = 23, LED3 = 24 , OFF = 25;

//Define System Criteria (All units in ISO)
...
bool Activate = true, Start = false;
const int quad_A = 2;
const int quad_B = 13;
const int IR1    = 7;
const int IR2    = 8;
...
double Kp1     =  150 * 468.085,
       Kp2     =  150 * 468.085,
       Kd      =   22 * 468.085,
       Kp_coll = 1000 * 468.085,
       Kd_coll =    0 * 468.085,
       Kprehab =   30 * 468.085;
double Acc1, w1, wold1 = 110, qold1 = 0, qd1 = 0, qe1 = 0, C1, S1, Cal1 = 1200, Command1, F1,
                 Acc2, w2, wold2 = 110, qold2 = 0, qd2 = 0, qe2 = 0, C2, S2, Cal2 = 1200, Command2, F2;
double q1 = 0, q2 = 0, q10 = pi / 3, q20 = 4 * pi / 6;
int Zcommand  = 1100;
int timescale = 100;
double Tint1, Tint2;

double QLD = 0.1745, QLU = 1.05, q1min = 0.45, q1max = 2.28, q2min = 0.98, q2max = 2.531;

double R = 0, R0 = 0.25, RX0 = 0, RY0 = 0;

String incomingByte;
int Type = 2;
bool Ok = false, circle = true, Flag = false;

double d, d0 = 0.001;
int  BX[8] = {0.077,  0.058, 0.004,  -0.06, -0.076, 723, 418, 0.06};
int  BY[8] = {    0, -0.043,  0.06, -0.044, -0.006, 529, 614, 0.04};
//290  -214
int i = 0, a1, a2;
int Bnumber = 0;
int counter_2 = 0;
...
void Calculate()
{
  ...
  */
  if (Activate && Start)
  {
    ...
    //Calculate the distance from the ball's center
    R = (X * X + Y * Y) ;

    switch (Type) {
      case 0://Turn Off the System
        F1 = 0; F2 = 0;
        break;

      case 1://Apply a spring and a damper
        F1 = -Kp1 * (X - X0) - Kd * v1;
        F2 =  Kp2 * (Y - Y0) - Kd * v2;
        break;

      case 2://Apply Ball Collision
        if (R < R0) {
          F1 =  Kp_coll * (X) - Kd_coll * v1;
          F2 = -Kp_coll * (Y) - Kd_coll * v2;
        }
        else {
          F1 =  0; F2 =  0;
        }
      ...
      case 3://Apply nueromoto-rehabitation
        Pull_to_Circle();
        //Stop_all_Motors();
        break;
    ...
    if (Safety) {
      ...
      //Send Commands to stop all motors
      Stop_all_Motors();
    }
    /*
      Commands Guide
...
bool Safety()
{
  if (q1 < q1min || q1 > q1max || q2 < q2min || q2 > q2max) return false;
  if ((q1 - q2) < QLD || (q1 - q2) > QLU) return false;
...
void Commands(double Ca1, double Ca2)
{
  //Send Commands
  // changing duty in pwm output pin 35
...
void Pull_to_Circle()
{
  ...
  {
    if (circle) {
      F1 = -Kprehab * (X - X0 + 0.077) - Kd * v1;
      F2 =  Kprehab * (Y - Y0 ) - Kd * v2;
    }
    else {
      F1 = -Kprehab * (X - X0 - 0.0486) - Kd_coll * v1;
      F2 =  Kprehab * (Y - Y0 - 0.0046) - Kd_coll * v2;
    }
  }
  //d = sqrt(pow((X - BX[Bnumber]), 2) + pow((Y - BY[Bnumber]), 2));
  //if (d < d0) Bnumber++;
  //F1 =  Kp1 * (X - X0 - BX[Bnumber]) - Kd_coll * v1;
  //F2 = -Kp2 * (Y - Y0 - BY[Bnumber]) - Kd_coll * v2;
  //F1 = -Kp_coll * (X - X0 + 0.077) - Kd_coll * v1;
  //F2 =  Kp_coll * (Y - Y0 + 0) - Kd_coll * v2;
}
void Calibration()
{
  Activate = false;
...
void loop()
{
  ...
  if (Start)
  {
    ...
    //Serial.println((String)F1 + " = F1  " + F2 + " = F2 ");
    delay(50);
  }
  ...
  /*
    ...
    Serial.print((String) "   Error 1 = " + qe1 + "   Position 1 = " + (180 / 3.14)*q1);
    Serial.print((String)"   Error 2 = " + qe2 + "   Position 2 = " + (180 / 3.14)*q2);
    ...
    Serial.println();
    Serial.println((String)"Kp = " + Kp1 / 468.085);
